...
export default function Hero() {
  ...
  const go = (direction) => {
    ...
    const next = direction === "next" ? (indexRef.current + 1) % 4 : (indexRef.current - 1 + 4) % 4;
    changeSlide(next);
  };
  ...
  const changeSlide = (nextIndex) => {
    ...
    tl.fromTo(imageRef.current, 
      ...
      { opacity: 1, scale: 1, y: 0, rotation: 0, duration: 0.8, ease: "power3.out" }
    )
  ...
  return (
    <section
      ...
      className={`relative w-full max-w-7xl mx-auto min-h-[85vh] md:min-h-[80vh] flex flex-col items-center justify-center overflow-hidden rounded-[2.5rem] shadow-2xl transition-colors duration-700 ease-in-out border ${borderColor} mt-4 mb-4`}
      style={{
    ...
    >
      <div ref={containerRef} className="absolute inset-0 w-full h-full pointer-events-none">
          {/* Background Elements */}
          <div className={`absolute top-1/4 left-1/4 w-96 h-96 rounded-full blur-[100px] opacity-30 animate-pulse ${isWhite ? "bg-gray-400" : "bg-white mix-blend-overlay"}`}></div>
          <div className={`absolute bottom-1/4 right-1/4 w-80 h-80 rounded-full blur-[80px] opacity-20 animate-pulse delay-1000 ${isWhite ? "bg-gray-300" : "bg-white mix-blend-overlay"}`}></div>
      </div>
      ...
      <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-8 w-full h-full px-6 md:px-16 py-12 md:py-0">
        ...
        <div className="flex flex-col items-center justify-center relative h-[80vh] md:h-auto">
          <img
            ...
            src={images[index]}
            alt={active.name}
            className="w-auto h-[60%] md:h-[70%] max-h-[500px] object-contain drop-shadow-[0_35px_35px_rgba(0,0,0,0.5)] z-20"
          />
          
          {/* Navigation Controls */}
          <div className="flex items-center gap-6 mt-8 md:mt-12 backdrop-blur-md bg-white/10 p-2 rounded-full border border-white/10 shadow-lg">
             <button onClick={() => go("prev")} className="p-3 hover:bg-white/20 rounded-full transition-colors active:scale-95 text-white">
             ...
             <button onClick={() => go("next")} className="p-3 hover:bg-white/20 rounded-full transition-colors active:scale-95 text-white">
        ...
        <div className={`flex flex-col justify-center items-start md:items-start ${textColor}`}>
          <div 
             ref={textRef}
          ...
          >
             ...
             <p className="text-sm font-bold tracking-[0.2em] uppercase mb-2 opacity-70">{active.name.replace("Flex ", "")} Edition</p>
             <h2 className="text-4xl md:text-5xl font-black mb-2 tracking-tight leading-none">{active.name}</h2>
             <p className="text-lg font-medium opacity-90 mb-6 italic">{active.tagline}</p>
